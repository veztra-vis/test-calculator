    ...
    try {
        ...
        if (!existing) {
            ...
            await users.updateOne(
                { employeeId: 'ancel' }, 
                { $set: { securityQuestion: 'What is your pet\'s name?', securityAnswer: 'maya' } }
            );
        }
    } catch (e) {
...
app.post('/api/auth/verify-security', async (req, res) => {
    ...
    try {
        var user = await (await getDb()).collection('users').findOne({ employeeId: employeeId });
        if (!user || !user.securityAnswer || answer !== user.securityAnswer.toLowerCase()) {
...
app.get('/api/auth/users', authMiddleware, async (req, res) => {
    ...
    try {
        const db = await getDb();
        res.json((await db.collection('users').find({}).toArray()).map(u => ({
            id: u.id, employeeId: u.employeeId, fullName: u.fullName, role: u.role, createdAt: u.createdAt
        })));
    } catch (e) {
        res.status(500).json({ error: 'DB error' });
    }
...
app.post('/api/auth/users', authMiddleware, async (req, res) => {
    ...
    try {
        const db = await getDb();
        if (await db.collection('users').findOne({ employeeId: employeeId.toLowerCase() })) {
            return res.status(409).json({ error: 'ID exists' });
        }
        var newUser = {
            id: 'USR' + Date.now().toString().slice(-6),
            employeeId: employeeId.toLowerCase(),
            fullName, password, role, securityQuestion,
    ...
    try {
        await fetch('https://api.emailjs.com/api/v1.0/email/send', {
            ...
            body: JSON.stringify({
                ...
                template_params: {
                    to_email, to_name: to_name || 'Customer', from_name: from_name || req.user.name, subject,
                    receipt_html: receipt_html || '', receipt_summary: receipt_summary || '',
                    total_amount: total_amount || 'N/A', principal: principal || 'N/A',
                    interest_total: interest_total || 'N/A', installments: installments || 'N/A'
                }
            })
        });
...
// ==========================================
// INTERNAL COMMUNICATION API (FLOOR COMMS)
// ==========================================
...
app.listen(PORT, async () => {
    ...
    await seedAdmin();
        ...
        console.error('DB Seed Error:', e.message); 
        // Do NOT exit process, let it retry when API is called
    }
}
...
app.post('/api/auth/login', async (req, res) => { var employeeId = (req.body.employeeId || '').toLowerCase(); var password = req.body.password; if (!employeeId || !password) return res.status(400).json({ error: 'Missing fields' }); try { const db = await getDb(); var user = await db.collection('users').findOne({ employeeId: employeeId }); if (!user || user.password !== password) return res.status(401).json({ error: 'Invalid Employee ID or Password' }); res.json({ token: generateToken(user), user: { employeeId: user.employeeId, fullName: user.fullName, role: user.role } }); } catch (e) { res.status(500).json({ error: 'Database error' }); } });
...
app.use(express.static(path.join(__dirname, 'public')));
var PORT = process.env.PORT || 10000;
app.listen(PORT, async () => { console.log('Maya app running on port ' + PORT); await seedAdmin(); });    if (allowedIps.includes(cleanIp)) { next(); } else { res.status(403).send('Access Denied'); }
});
